...
def main():
    ...
    categories_touse = ["['Mass']", "['Suspicious Calcification', 'Mass']"]
    metadata = metadata[metadata['finding_categories'].isin(categories_touse)]

    # paths
    saving_tag = 'wMass'
    new_images_folder_path = repo_path / 'data/vindr-mammo/images'/f'{folder_name}_RGB_{saving_tag}' # where the images are going to
    masks_folder_path = repo_path / 'data/vindr-mammo/masks'/f'{folder_name}_RGB_{saving_tag}' # where the masks are going to
    ...
    for j, row in metadata.iterrows(): # go over all images with lesion  
        ...
        if np.any(coord < 0) or (coord[0]-coord[2]) == 0:
            tqdm_bar.update()
            bad_image_count += 1
            print(f'bad image {file_name} count is now {bad_image_count} because coord is {coord}')
            continue
