...
def main():
    ...
    config_path = exp_path / 'config_files/latent_fine.yaml' # configuration file path (beter to call it from the args parser)
    ...
    for epoch in range(num_epochs): # Loop over the epochs
        ...
        for latents in train_dataloader: # Loop over the batches
            with accelerator.accumulate(model): # start gradient accumulation
                ...
                noise_pred = model(noisy_images, timesteps, encoder_hidden_states='').sample # sample tensor
                # Calculate the loss
