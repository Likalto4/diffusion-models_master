...
def main():
    ...
    config_path = exp_path / 'config_files/latent_32.yaml'
    with open(config_path, 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
    # name the model and get the full name in HF Hub
    hub_model_id = get_full_repo_name(config['saving']['local']['pipeline_name']) #saving now with same pipeline name  # (config['saving']['hf']['repo_name'])
    ...
    with open(exp_path / 'model_cards/latent.yaml', 'r') as f:
        content = f.read()
